...
const C = {
  ...
  clay: "#9B8C9E",     // Lavendel (Haupt-Akzent)
  clayDeep: "#8A7888", // Quarz (Hover & Sekundär-Akzent)
  moss: "#3C3038",     // Nacht (Dunkler Hintergrund)
  mossMid: "#8A7888",  // Quarz
  dust: "#C0B4C0",     // Helles Lavendel (Linien, zarte Akzente)
  midnight: "#2E242B", // Tiefes Nacht (Footer)
...
body{background:#FDFAF5;color:#3C3038;font-family:'DM Sans',sans-serif;font-weight:300;line-height:1.7}
...
.wk-grid-kontakt > div { border-left:1px solid rgba(60,48,56,.07); padding-left:2.5rem; padding-right:2.5rem; }
...
@media(max-width:900px){
  ...
  .wk-grid-kontakt > div { border-left:none; border-bottom:1px solid rgba(60,48,56,.07); padding:0 0 2.5rem 0; }
    ...
    <div style={wrap} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} onClick={() => inp.current?.click()}>
      ...
      {img ? (
        <>
          ...
          {hov && <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: ".6rem 1rem", background: "rgba(60,48,56,.7)", backdropFilter: "blur(6px)" }}>
...
function Btn({ children, onClick, variant = "solid", light = false }) {
  ...
  const v = {
    ...
    outline: { background: hov ? (light ? "rgba(253,250,245,.08)" : "rgba(155,140,158,.07)") : "transparent", color: light ? C.white : C.ink, border: `1.5px solid ${light ? C.dust : C.clay}` },
  };
  return (
  ...
  return (
    <div style={{ borderBottom: `1px solid ${C.stone}` }}>
      <div onClick={() => setOpen(!open)} style={{ cursor: "pointer", padding: "1.3rem 0", display: "flex", justifyContent: "space-between", alignItems: "center", gap: "1.5rem" }}>
        <span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "1.1rem", color: C.ink, lineHeight: 1.3 }}>{q}</span>
      ...
      <div style={{ maxHeight: open ? "500px" : "0", overflow: "hidden", transition: "max-height .5s ease" }}>
        <p style={{ paddingBottom: "1.3rem", fontSize: ".87rem", color: C.ink, lineHeight: 1.85, paddingRight: "1.5rem" }}>{a}</p>
      </div>
    </div>
  );
}
function Badge({ children }) {
  ...
  return (
    <span onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{ display: "inline-block", border: `1px solid ${hov ? C.clay : C.stone}`, borderRadius: "20px", padding: ".38rem 1rem", fontSize: ".77rem", color: hov ? C.white : C.ink, margin: ".28rem", background: hov ? C.clay : C.white, transition: "all .2s" }}>
      {children}
    </span>
  );
}
  ...
  return (
    <div style={{ position: "relative", width: size, height: size, flexShrink: 0, pointerEvents: "none" }}>
      ...
      <div style={{
        ...
        inset: "20%",
        borderRadius: "50%",
        background: `radial-gradient(ellipse at 50% 50%, rgba(155,140,158,.15) 0%, rgba(138,120,136,.08) 40%, transparent 65%)`,
...
function SvcCard({ icon, title, desc, onClick }) {
  ...
  return (
    <div onClick={onClick} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      ...
      <h3 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "1.4rem", fontWeight: 400, color: C.ink, marginBottom: ".6rem" }}>{title}</h3>
      <p style={{ fontSize: ".81rem", lineHeight: 1.75, color: C.ink, margin: 0, flexGrow: 1 }}>{desc}</p>
...
function OilCard({ name, lat, anw, kind }) {
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      ...
      <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "1.7rem", color: C.ink, marginBottom: ".3rem" }}>{name}</div>
      <div style={{ fontFamily: "'DM Mono',monospace", fontSize: ".65rem", color: C.clay, marginBottom: "1.2rem", fontStyle: "italic" }}>{lat}</div>
      <p style={{ fontSize: ".85rem", color: C.ink, margin: "0 0 1.2rem", lineHeight: 1.6, flexGrow: 1 }}>{anw}</p>
  ...
  return (
    <>
      ...
      <section style={{ background: C.moss, position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", inset: 0, background: `radial-gradient(ellipse 70% 60% at 65% 45%, rgba(155,140,158,.25) 0%, transparent 65%)`, pointerEvents: "none" }} />
        <div className="wk-hero-grid" style={{ maxWidth: "1380px", margin: "0 auto", position: "relative", zIndex: 2 }}>
          <div style={{ display: "flex", flexDirection: "column", justifyContent: "flex-end", padding: "clamp(4rem,8vw,7rem) clamp(1.4rem,5vw,5rem) clamp(3.5rem,6vw,5.5rem)" }}>
            ...
            <h1 style={{ fontFamily: "'Cormorant Garamond',serif", fontWeight: 300, lineHeight: 1.0, color: C.white, marginBottom: "2rem", fontSize: "clamp(2.5rem,5.5vw,6.8rem)" }}>
              ...
              <span style={{ fontSize: "clamp(2rem,3.8vw,4.6rem)", color: C.cream, fontStyle: "italic" }}>— und du auch.</span>
            </h1>
          ...
          <div className="wk-hero-right" style={{ display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
            <div style={{ position: "absolute", inset: 0, background: `radial-gradient(ellipse 80% 60% at 60% 40%, rgba(138,120,136,.15) 0%, transparent 65%)`, pointerEvents: "none" }} />
            <BreathingCircle size={400} />
            <div style={{ position: "absolute", bottom: "2.5rem", right: "2.5rem", fontFamily: "'DM Mono',monospace", fontSize: ".57rem", color: C.dust, letterSpacing: ".1em", textAlign: "right", lineHeight: 2.2 }}>
      ...
      <section className="wk-section" style={{ background: C.white }}>
        <div className="wk-inner wk-2col" style={{ alignItems: "center" }}>
          <Reveal>
            ...
            <p style={{ fontSize: ".93rem", lineHeight: 1.95, color: C.ink, marginBottom: "1.2rem" }}>
            ...
            <p style={{ fontSize: ".93rem", lineHeight: 1.95, color: C.ink, marginBottom: "2rem" }}>
          ...
          <Reveal delay={0.15}>
            <div style={{ position: "relative", maxWidth: "420px", margin: "0 auto" }}>
              ...
              <div style={{ position: "absolute", bottom: "-1.4rem", right: "-1.4rem", background: C.cream, border: `1px solid ${C.stone}`, padding: "1.3rem 1.8rem", maxWidth: "210px" }}>
                ...
                <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: ".95rem", color: C.ink, lineHeight: 1.35 }}>Erfahrung mit Kindern & Familien</div>
              </div>
            </div>
          </Reveal>
        </div>
      ...
      <section className="wk-section" style={{ background: C.moss, position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", top: "-10%", right: "-5%", width: "480px", height: "480px", borderRadius: "50%", border: `1px solid rgba(155,140,158,.12)`, pointerEvents: "none" }} />
      ...
      <section className="wk-section" style={{ background: C.cream, borderTop: `1px solid ${C.stone}` }}>
        <div className="wk-inner wk-2col" style={{ alignItems: "center" }}>
          <Reveal>
            ...
            <p style={{ fontSize: ".95rem", color: C.ink, lineHeight: 1.85, maxWidth: "420px", marginBottom: "2.5rem" }}>
            ...
            <div style={{ border: `1px solid ${C.stone}`, background: C.white }}>
              {[
                ...
                <div key={i} style={{ padding: "2rem 2.5rem", borderBottom: i < 2 ? `1px solid ${C.stone}` : "none" }}>
                  ...
                  <div style={{ fontSize: ".8rem", color: C.ink }}>{item.sub}</div>
                </div>
              ))}
            </div>
          </Reveal>
    ...
    <>
      <section style={{ background: C.moss, paddingTop: "70px", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: "42%", zIndex: 0 }}>
          ...
          <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to right,#3C3038 0%,rgba(60,48,56,.2) 100%)", pointerEvents: "none" }} />
        </div>
      ...
      <section className="wk-section"><div className="wk-inner wk-2col">
        <Reveal>
          ...
          <p style={{ fontSize: ".92rem", lineHeight: 1.9, color: C.ink, marginBottom: "1.2rem" }}>Wenn das Baby viel weint, schlecht schläft, beim Trinken unruhig ist oder sich stark überstreckt, wächst die Sorge schnell: "Mache ich etwas falsch? Hat mein Kind Schmerzen?"</p>
          <p style={{ fontSize: ".92rem", lineHeight: 1.9, color: C.ink, marginBottom: "1.2rem" }}>Du machst nichts falsch. Manchmal hinterlässt der Weg ins Leben — eine rasante Geburt, ein Kaiserschnitt, der Einsatz einer Saugglocke — Spannungen im winzigen Körper.</p>
          <div style={{ padding: "1.5rem 2rem", background: C.cream, borderLeft: `3px solid ${C.clay}`, marginTop: "1rem" }}>
            <h4 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "1.2rem", color: C.ink, marginBottom: ".5rem" }}>Sicherer Hafen für Mütter</h4>
            <p style={{ fontSize: ".82rem", color: C.ink, lineHeight: 1.75, margin: 0 }}>In meiner Praxis herrscht kein Zeitdruck. Wenn gestillt, gewickelt oder gekuschelt werden muss, machen wir Pause.</p>
          </div>
        </Reveal>
        <Reveal delay={0.2}>
      ...
      <section className="wk-section-s" style={{ background: C.cream }}><div className="wk-inner">
        <Reveal><Label n="02" text="Wann kommen Familien zu mir" /><H2>Was Eltern häufig berichten.</H2>
          <p style={{ fontSize: ".88rem", color: C.ink, lineHeight: 1.85, maxWidth: "620px", marginBottom: "2rem" }}>Die craniosacrale Therapie ist eine ergänzende Methode — kein Ersatz für ärztliche Diagnosen.</p>
        </Reveal>
            ...
            <Reveal key={g.g} delay={i * 0.12} style={{ height: "100%" }}>
              <div style={{ padding: "2.3rem 1.8rem", height: "100%" }}>
                ...
                <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
                  {g.items.map(it => (
                    <li key={it} style={{ fontSize: ".84rem", color: C.ink, padding: ".4rem 0", borderBottom: `1px solid ${C.stone}`, lineHeight: 1.5 }}>
      ...
      <section className="wk-section-s" style={{ background: C.moss }}><div style={{ maxWidth: "680px", margin: "0 auto" }}>
        <Reveal>
          ...
          <div style={{ border: `1px solid rgba(221,212,200,.2)`, marginTop: "2rem" }}>
            {[["Erstbehandlung Säugling (60 min)", "€ 75,–"], ["Folgebehandlung (45 min)", "€ 60,–"], ["Erstbehandlung Kleinkind / Erwachsener", "€ 80,–"], ["CST + Tuina kombiniert", "€ 85,–"]].map(([l, p], i) => (
              <div key={i} style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: "1rem", alignItems: "center", padding: "1.1rem 1.8rem", borderBottom: "1px solid rgba(221,212,200,.15)", background: i % 2 === 0 ? "rgba(255,255,255,.03)" : "transparent" }}>
                <span style={{ fontSize: ".87rem", color: C.stone }}>{l}</span>
                <span style={{ fontFamily: "'DM Mono',monospace", fontSize: ".84rem", color: C.clay }}>{p}</span>
              </div>
            ))}
          </div>
          <p style={{ fontSize: ".73rem", color: C.stone, marginTop: "1rem", lineHeight: 1.7 }}>Barzahlung oder Honorarnote für private Zusatzversicherungen.</p>
        </Reveal>
      </div></section>
    </>
  );
    ...
    <>
      ...
      <section className="wk-section"><div className="wk-inner wk-2col">
        <Reveal>
          ...
          <p style={{ fontSize: ".9rem", lineHeight: 1.9, color: C.ink, marginBottom: "1.2rem" }}>Mit einfachen, alltagstauglichen Rezepten — wärmende Kraftsuppen, spezielle Porridges — füllen wir deine Speicher auf, unterstützen den Milchfluss und helfen dem Körper, das Erschöpfungs-Plateau nach der Geburt zu überwinden.</p>
        ...
        <Reveal delay={0.2}>
          ...
          {[
            ...
            <div key={i} style={{ padding: "1rem 0", borderBottom: `1px solid ${C.stone}` }}>
              ...
              <p style={{ fontSize: ".82rem", color: C.ink, margin: 0, lineHeight: 1.75 }}>{s.d}</p>
            </div>
          ))}
        </Reveal>
      </div></section>
    ...
    <>
      ...
      <section className="wk-section"><div className="wk-inner wk-2col">
        <Reveal>
          ...
          <p style={{ fontSize: ".9rem", lineHeight: 1.9, color: C.ink, marginBottom: "1.2rem" }}>Als ausgebildete Kursleitung gebe ich Eltern konkrete Griffe mit, die sie täglich zu Hause anwenden können — ein echtes Werkzeug für den Alltag, das die Bindung stärkt.</p>
          ...
          <div style={{ marginBottom: "1.5rem" }}>
            {["Koliken & Bauchschmerzen", "Verstopfung", "Häufige Erkältungen", "Schlafstörungen", "Zahnen", "Immununterstützung", "Atemwegsinfekte begleiten"].map(s => (
              <span key={s} style={{ display: "inline-block", border: `1px solid ${C.stone}`, borderRadius: "20px", padding: ".33rem .9rem", fontSize: ".75rem", color: C.ink, margin: ".22rem", background: C.white }}>{s}</span>
            ))}
          </div>
      ...
      <div className="wk-inner wk-2col" style={{ marginTop: "4rem", paddingTop: "4rem", borderTop: `1px solid ${C.stone}` }}>
        ...
        <Reveal delay={0.2}>
          ...
          <p style={{ fontSize: ".9rem", lineHeight: 1.9, color: C.ink, marginBottom: "1.5rem" }}>Besonders für Mütter, deren Schulter-Nacken-Bereich durch Tragen und Stillen verspannt ist, bietet das sanfte Trocken-Schröpfen spürbare Erleichterung.</p>
          {["Löst tiefe Muskelverspannungen und Faszienverklebungen.", "Fördert die lokale Durchblutung.", "Ergänzt die Tuina-Behandlung wirkungsvoll."].map((s, i) => (
            <div key={i} style={{ display: "flex", gap: "1rem", padding: ".75rem 0", borderBottom: `1px solid ${C.stone}`, fontSize: ".82rem", color: C.ink, lineHeight: 1.7 }}>
    ...
    <>
      ...
      <section className="wk-section"><div className="wk-inner wk-2col">
        <Reveal>
          ...
          <p style={{ fontSize: ".9rem", lineHeight: 1.9, color: C.ink, marginBottom: "1.5rem" }}>Der G-Well Pointer lokalisiert Akupunkturpunkte und Triggerpunkte durch Messung des Hautwiderstands und stimuliert sie mit sanften mikroelektrischen Impulsen — vollständig schmerzfrei.</p>
          {[{ n: "01", t: "Absolut schmerzfrei", d: "Ideal für alle, die Angst vor Nadeln haben." }, { n: "02", t: "Narbenentstörung nach Kaiserschnitt", d: "Besonders wirksam bei Narbengewebe — relevant für Mütter nach Kaiserschnitt." }, { n: "03", t: "Direkte Entspannung", d: "Wirkung bei stark verspanntem Nacken- und Schulterbereich." }].map(s => (
            <div key={s.n} style={{ padding: "1.1rem 1.4rem", background: C.cream, borderLeft: `3px solid ${C.clay}`, marginBottom: ".8rem" }}>
              ...
              <div style={{ fontSize: ".83rem", fontWeight: 500, color: C.ink, marginBottom: ".2rem" }}>{s.t}</div>
              <p style={{ fontSize: ".78rem", color: C.ink, margin: 0, lineHeight: 1.7 }}>{s.d}</p>
            </div>
          ))}
        </Reveal>
    ...
    <>
      ...
      <section className="wk-section"><div className="wk-inner wk-2col">
        <Reveal>
          ...
          <p style={{ fontSize: ".9rem", lineHeight: 1.9, color: C.ink, marginBottom: "1rem" }}>
    ...
    <>
      <section className="wk-about-hero" style={{ background: C.moss }}>
        ...
        <div style={{ position: "relative", minHeight: "420px" }}>
          ...
          <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to right,rgba(60,48,56,.8) 0%,transparent 40%)", pointerEvents: "none" }} />
        </div>
      </section>
      <section className="wk-section"><div className="wk-inner wk-2col">
        <Reveal>
          ...
          <p style={{ fontSize: ".9rem", lineHeight: 1.9, color: C.ink, marginBottom: "1.1rem" }}>Mein Weg zur craniosakralen Therapie begann mit der Faszination für die Weisheit des menschlichen Körpers. Jeder Mensch — vom ersten Atemzug an — trägt die Fähigkeit zur Selbstregulation in sich.</p>
          <p style={{ fontSize: ".9rem", lineHeight: 1.9, color: C.ink, marginBottom: "1.1rem" }}>Als ehemalige Leiterin der Kinderstation des Krankenhauses Eisenstadt habe ich Tausende von Kindern begleitet und Familien in den schwierigsten Momenten ihres Lebens gestützt.</p>
          <p style={{ fontSize: ".9rem", lineHeight: 1.9, color: C.ink }}>In meiner Praxis in Mattersburg biete ich einen geschützten Raum — einen Raum, in dem Erschöpfung sein darf, Tränen fließen dürfen und wir gemeinsam die Wurzeln für ein gesundes Wachstum stärken.</p>
        </Reveal>
        <Reveal delay={0.2}>
          ...
          {quals.map((q, i) => (
            <div key={i} style={{ padding: ".9rem 0", borderBottom: `1px solid ${C.stone}`, display: "flex", gap: "1rem" }}>
              ...
              <div>
                <div style={{ fontSize: ".76rem", fontWeight: 500, color: C.ink, marginBottom: ".12rem" }}>{q.t}</div>
                <div style={{ fontSize: ".74rem", color: C.ink, lineHeight: 1.6 }}>{q.s}</div>
              </div>
            </div>
          ))}
        </Reveal>
    ...
    <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, background: scrolled || open ? "rgba(253,250,245,.97)" : "rgba(253,250,245,.88)", backdropFilter: "blur(14px)", borderBottom: `1px solid rgba(155,140,158,${scrolled ? .2 : .08})`, transition: "all .3s" }}>
      <div style={{ maxWidth: "1400px", margin: "0 auto", padding: "0 clamp(1.2rem,3vw,2.5rem)", height: "70px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div className="wk-nav-desk">
          {items.map(n => (
            <button key={n.id} onClick={() => go(n.id)}
              style={{ background: "none", border: "none", fontSize: ".62rem", letterSpacing: ".13em", textTransform: "uppercase", color: current === n.id ? C.clay : C.ink, padding: ".5rem .7rem", transition: "color .2s", borderBottom: current === n.id ? `2px solid ${C.clay}` : "2px solid transparent", fontFamily: "'DM Sans',sans-serif", fontWeight: current === n.id ? 500 : 300 }}>
              {n.short}
            </button>
          ))}
        ...
        <button className="wk-nav-mob" onClick={() => setOpen(!open)} style={{ background: "none", border: "none", width: 32, height: 32, flexDirection: "column", justifyContent: "center", gap: "5px", padding: "4px" }}>
          {[0, 1, 2].map(i => <span key={i} style={{ display: "block", height: "1.5px", background: C.ink, transition: "all .25s", opacity: open && i === 1 ? 0 : 1, transform: open ? (i === 0 ? "translateY(6.5px) rotate(45deg)" : i === 2 ? "translateY(-6.5px) rotate(-45deg)" : "none") : "none" }} />)}
        </button>
      </div>
      {open && (
        <div style={{ background: C.white, borderTop: `1px solid ${C.stone}`, padding: "1.5rem 1.4rem 2rem" }}>
          {items.map(n => (
            <button key={n.id} onClick={() => go(n.id)} style={{ display: "block", width: "100%", textAlign: "left", background: "none", border: "none", fontSize: ".85rem", letterSpacing: ".1em", textTransform: "uppercase", color: current === n.id ? C.clay : C.ink, padding: ".9rem 0", borderBottom: `1px solid ${C.stone}`, fontFamily: "'DM Sans',sans-serif" }}>{n.short}</button>
          ))}
